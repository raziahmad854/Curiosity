...
async def search_image(query: str, width: int = 800, height: int = 500) -> str:
    ...
    Priority: Pexels API (needs key) → Wikimedia Commons (free) → loremflickr fallback."""
    ...
    # 3) Final fallback: loremflickr returns keyword-matched images (no key needed)
    encoded = query.replace(" ", ",")
    return f"https://loremflickr.com/{width}/{height}/{encoded}?lock={hash(query) % 1000}"
